...
export const createUser = async (req, res) => {
    ...
    try {
        const newUser = new User({ username, email, _id: id });
        await newUser.save();
...
export const followUser = async (req, res) => {
    ...
    try {
        ...
        // Logique pour suivre la tâche (à implémenter)
        if (followedUser.followers.includes(followedUserId)) {
            return res.status(400).json({ error: 'You are already following this user' });
        }
        res.status(200).json({ message: `User ${followedUserId} followed user ${req.params.id}` });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
};