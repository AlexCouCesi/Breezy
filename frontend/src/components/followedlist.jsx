    ...
    if (!followingList || followingList.length === 0) {
        ...
        return (
            ...
            <ul className="space-y-4">
                ...
                <li key={user._id} className="flex items-center gap-3 justify-between">
                    <Link href={`/profile/${user._id}`} className="flex items-center gap-3 hover:underline">
                        <img
                            src={user.profilePicture ? `${process.env.NEXT_PUBLIC_API_URL}/${user.profilePicture}` : `${process.env.NEXT_PUBLIC_API_URL}/assets/icones_divers/profile_icon.png`}
                            alt={user.username}
